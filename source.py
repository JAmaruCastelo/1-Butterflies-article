...
### en base a dos criterios diferentes, para poder seleccionar las especies conocidas 
...
def grafi_general_r_a (por_tipo, tipos=["CCR", "PCR", "SLR"]):
    ...
    for nombre in indice2 :
        nombre=str(nombre)
        yubication = a[nombre]
        ax1.annotate(nombre,(z-1, yubication+0.1), rotation=90,fontsize=8)
        z=z+1
    ...
    for nombre in indice2 :
        nombre=str(nombre)
        yubication = b[nombre]
        ax2.annotate(nombre,(z-1, yubication+0.1), rotation=90,fontsize=8)
        z=z+1
    ...
    for nombre in indice2 :
        nombre=str(nombre)
        yubication = c[nombre]
        ax3.annotate(nombre,(z-1, yubication+0.1), rotation=90,fontsize=8)
        z=z+1
    return fig
...
def out_df(df): # quito el intercuartil
    uno=df.quantile(0.25)
    tres=df.quantile(0.75)
    IQR=tres-uno
    df_no_out=df[~((df<(uno-1.5*IQR))|(df>(tres+1.5*IQR)))]
    return df_no_out
